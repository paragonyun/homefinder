...
- 실패: `TMAP_API_KEY` 없음, 좌표/주소 없음, 권한 없음, TMAP API 오류를 명시적으로 반환합니다. 자동차만 동작하고 대중교통이 권한 오류를 반환하면 기존 appKey에 대중교통 API 상품 권한을 추가하거나, 대중교통 권한이 있는 키를 `TMAP_TRANSIT_API_KEY`로 분리 등록합니다.
